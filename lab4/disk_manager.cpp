...
void DiskManager::create_file(const std::string &path) {
    ...
    // 为了注意不创建相同文件,应该先检查文件是否存在。可以调用上面的is_file
    if(!is_file(path)) {
        // 这边创建的时候要第三个参数设置读写权限，不然后面open全部失败返回-1，因为没有权限打开。伞兵bug卡我一整天
        // https://www.cnblogs.com/pswzone/archive/2012/04/14/2446623.html 《open创建文件后，再读取出现Permission denied错误》
...
void DiskManager::destroy_file(const std::string &path) {
    ...
    // 为了不能删除未关闭的文件，我们应该先检查文件是不是关闭的。是关闭的，才删除
    // 文件打开列表有两个哈希表，用于记录打开的文件。可以用path2fd_检查，效仿get_file_fd
    if (!is_file(path)) {
...
int DiskManager::open_file(const std::string &path) {
    ...
    // 注意get_file_fd的实现，当文件未打开时直接返回了open_file的返回值，因此要返回句柄
    // 要更新两个打开文件列表
    // 不能重复打开，所以应该先检查.
    if(path2fd_.count(path)) {
    ...
    // 文件尚未打开，才会走下面代码打开.
    // 纠错：最早忘了检查文件是否创建。打开文件之前，还应该确保文件已经创建。用is_file检查文件是否存在
    if(!is_file(path)) {
        throw FileNotFoundError(path);
...
void DiskManager::close_file(int fd) {
    ...
    // 先检查文件是否打开,通过fd2path_检查,若已经打开，就关闭
    if(fd2path_.count(fd)) {
        close(fd);
        path2fd_.erase(fd2path_[fd]);  // 先删除path2fd_中的项
        fd2path_.erase(fd);  // 再删除fd2path_中的项
    } else {
